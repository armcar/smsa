...
return [
    ...
    'member_area_shared_secret' => env(
        'SMSA_MEMBER_API_SHARED_SECRET',
        env('WP_MEMBER_AREA_SHARED_SECRET', env('SMSA_INGEST_SHARED_SECRET', env('WP_LARAVEL_BRIDGE_TOKEN', '')))
    ),
    'member_area_previous_shared_secrets' => array_values(array_filter(array_map(
        static fn (string $secret): string => trim($secret),
        explode(',', (string) env('SMSA_MEMBER_API_PREVIOUS_SHARED_SECRETS', ''))
    ))),
